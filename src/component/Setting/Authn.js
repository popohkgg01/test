...
export default function Authn(props) {
    ...
    const deleteCredential = (id) => {
        API.patch("/user/setting/authn", {
            ...
            .then(() => {
                ToggleSnackbar("top", "right", "憑證已刪除", "success");
                props.remove(id);
            })
            .catch((error) => {
    ...
    const addCredential = () => {
        if (!navigator.credentials) {
            ToggleSnackbar("top", "right", "目前瀏覽器或環境不支援", "warning");
        ...
        API.put("/user/authn", {})
            ...
            .then((response) => {
                ...
                ToggleSnackbar("top", "right", "驗證器已新增", "success");
                return;
            })
            .catch((error) => {
                console.log(error);
    ...
    return (
        <div>
            <Dialog open={confirm} onClose={() => setConfirm(false)}>
                <DialogTitle>刪除憑證</DialogTitle>
                <DialogContent>確定要吊銷這個憑證嗎？</DialogContent>
                <DialogActions>
